...
buf1 = np.zeros((DIM1), dtype=np.double)


fid1 = h5py.h5f.create(FILE1)
#dims = (DIM1, DIM2)
dims = (DIM1,0)
# todo do we need create_simple?
# num dimensions , size of each dimensions
space1 = h5py.h5s.create_simple(dims)
#space1 = h5py.h5s.create(dims)
...
# todo is this len(frames) or totalframes
...
coord = np.zeros(len(frames))
#coord = np.zeros((len(frames),2))
#coord = np.zeros((len(frames)), dtype=object)

maxRange = len(frames)
global numOffsetBlanksToAdd

print ('coord', coord)
for i in range(maxRange):
   #newVal = [i+numOffsetBlanksToAdd,0]
   #coord[i] = np.reshape(newVal)
   coord[i] = i+numOffsetBlanksToAdd
#coord = np.reshape(coord,maxRange)

#np.reshape(dset1, DIM1)
...
convertCoord = np.zeros(len(frames))

for i in range(maxRange):
   #newVal = [i+numOffsetBlanksToAdd,0]
   #coord[i] = np.reshape(newVal)
   convertCoord[i] = [i+numOffsetBlanksToAdd,0]
...
bufnew = np.zeros((DIM1), dtype=np.double)

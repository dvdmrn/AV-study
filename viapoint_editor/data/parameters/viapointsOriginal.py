...
class ViaPointsParameter(IInteractiveParameter):
    ...
    def construct_region(self, vpts, index, x, y, inputdata):
        # Takes mouse release coordinates and passes them to move the viapoints then passes this to apply paramater
        Xnew, Xnewd, Xnewdd, Xnewddd, vpts = self.vp.move_via_point(vpts, index, x, y)
        self.constructed_region = Xnew

        self.applyParameter(inputdata)
    ...
    def applyParameter(self, data):
        ...
        ## TODO pass this updated data into already rendered plot

        return_data[start:stop+1] = self.constructed_region
...
class InteractivePlot(QtGui.QDialog):
    ...
    def __init__(self, parameter, dataInput, dataNow, dataAudio, parent=None):
        ...
        # import other class instances into GUI
        self.vppr = ViaPointsParameter()
        self.vp = ViaPoints()
        ...
        self.button_move.clicked.connect(self.drag_points)
    ...
    def button_press_callback(self, event):
        ...
        ## TODO take x value of mousepress and match it to closest x value from vpts x and y coordinates? (FE)
        ## TODO take the index and feed it into moveviapoints() function (BE)
        ## TODO take this and feed it into applyparameter(data) (BE)
        ## TODO reconstruct graph according to these new viapoints, BUT maintain the same view to able to see changes (FE)
        ## TODO Finish GUI mouse movement. so the point moves with the mouse
        ...
        M_point = min(self.via_x, key=lambda x:abs(x-event.xdata))

        indseq = np.where(self.via_x == M_point)
        ind = indseq[0][0]
        self.ind = ind

        #self.button_pressed.connect_event("motion_notify_event", self.motion_notify_callback)
        print ind
        self.button_pressed = widgets.AxesWidget(self.axes[0])
    ...
    def button_release_callback(self, event):
        ...
        self.vppr.construct_region(self.vpts, self.ind, event.xdata, event.ydata, self.datainput)



        # Xnew, Xnewd, Xnewdd, Xnewddd, vpts = self.vp.move_via_point(self.vpts, self.ind, event.xdata, event.ydata)
        # self.vppr.applyParameter(self.datainput)


    # def motion_notify_callback(self, event):
    #     y = event.ydata
    #     # self.datanow[self.ind] = y
    #     self.DVPTS[:, 0][self.ind] = y
    #     self.axes[0].plot(self.via_x, self.DVPTS[:, 0], 'go')
    #     self.canvas.draw()
...
class ViaPoints():
    ...
    def move_via_point(self, vpts, index, to_x, to_y):

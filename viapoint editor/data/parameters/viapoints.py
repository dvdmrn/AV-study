...
class ViaPointsParameter(IInteractiveParameter):
    ...
    def applyParameter(self, data):
        ...
        Moved_data = self.getData('Moved_data')


        #creates new stack of data
        return_data = data.__copy__()

        # Replaces altered data in old dataset
        return_data[start:stop] = Moved_data
...
class InteractivePlot(QtGui.QDialog):
    ...
    def __init__(self, parameter, dataInput, dataNow, dataAudio, parent=None):
        ...
        self.figure, self.axes = plt.subplots(4, figsize=(12, 8))
        ...
        self.button_save = QtGui.QPushButton('Consolidate')
        self.button_save.setEnabled(False)

        self.button_move.clicked.connect(self.start_move)
        self.button_save.clicked.connect(self.save_quit)
        self.button_via.clicked.connect(self.addSpanSelector)

        self.button_close = QtGui.QPushButton('Quit', parent=self)
        self.button_close.clicked.connect(self.close)

        self.button_clear = QtGui.QPushButton('Clear Span', parent=self)
    ...
    def clearRegion(self):
        ...
        self.button_via.setEnabled(True)
    ...
    def onSpanSelect(self, xmin, xmax):
        ...
        # Pass data without fitting original data
        Xp_want = np.hstack(Xp)
        ...
        # Pass X-coord without fitting to data
        x_pos = np.append(vpts[0].ts, data.size-1)
        ...
        self.mm = self.axes[0].get_ylim()
        
        self.axes[1].plot((via_x[0], via_x[0]), (self.mm[0], self.mm[1]), 'b-.')
        self.axes[1].plot((via_x[-1], via_x[-1]), (self.mm[0], self.mm[1]), 'b-.')

        self.axes[0].set_ylim(self.mm)

        self.canvas.draw()
        #self.button_move.clicked.connect(self.open_modify(Xp_final, via_x, vpts))

        self.button_clear.setEnabled(False)
        self.button_move.setEnabled(True)

        self.open_modify(Xp_want, x_pos, vpts, self.datanow)

    def open_modify(self, X, x_pos, vpts, RunningData):
        self.axes[1].clear()
        self.axes[1].plot(self.x_animation_range, RunningData, color='r', label="a")
        self.axes[1].plot((self.via_x[0], self.via_x[0]), (self.mm[0], self.mm[1]), 'b-.')
        self.axes[1].plot((self.via_x[-1], self.via_x[-1]), (self.mm[0], self.mm[1]), 'b-.')
        ...
        self.x_pos = x_pos
        self.vpts = vpts
        self.multi = widgets.MultiCursor(self.canvas, self.axes[1:4])
        self.axes[0].clear()
        self.axes[0].set_title("Viapoint edit Animation Data (" + str(self.X.size) + " samples)")
        self.axes[0].plot(X, 'b-.', x_pos, vpts[0].vp[:,0], 'go')
        self.axes[0].set_ylim(self.mm)
        self.canvas.draw()


    def start_move(self):
        # Registers mouse_click and Passes X_coordinates

        self.button_pressed = widgets.AxesWidget(self.axes[0])
        self.button_pressed.connect_event("button_press_event", self.button_press_callback)

    def button_press_callback(self, event):

        # CODE IN CONTEXT OF REST OF DATA

        # Identifies index of Viapoint closest to mouse-click
        M_point = min(self.x_pos, key=lambda x:abs(x-event.xdata))
        indseq = np.where(self.x_pos == M_point)
        ind = indseq[0][0]
        self.ind = ind


        self.button_pressed = widgets.AxesWidget(self.axes[0])
        self.button_pressed.connect_event("button_release_event", self.button_release_callback)

    def button_release_callback(self, event):

         Xnew, Xnewd, Xnewdd, Xnewddd, vpts = self.vp.move_via_point(self.X, self.vpts, self.ind, event.xdata, event.ydata)

         Xp_want = np.hstack(Xnew)
         x_pos = np.append(vpts[0].ts, Xp_want.size-1)

         self.button_pressed.disconnect_events()

         self.Xp_want = np.array(Xp_want, dtype=float)
         RunningData = self.datanow
         RunningData[self.indmin:self.indmax] = self.Xp_want



         self.open_modify(Xp_want, x_pos, vpts, RunningData)


    def save_quit(self):

        self.parameter.setData('Moved_data', self.Xp_want)
        self.close()
    ...
    def plotAllDataSpan(self):
        ...
        if (self.span == None):
            self.span = widgets.SpanSelector(self.axes[0], self.onSpanSelect, 'horizontal', useblit=True, rectprops=dict(alpha=0.5, facecolor='lightskyblue'), span_stays=True)
    ...
    def plotAllData(self):
        ...
        plt_animation, = self.axes[0].plot(self.x_animation_range, self.datanow, color='r', label='Animation Data')
        self.axes[0].set_xlim([0, self.x_animation_range[-1]])
        self.axes[0].set_title("Viapoint Animation Data (" + str(self.x_animation_range.size) + " samples)")

        plt_animation, = self.axes[1].plot(self.x_animation_range, self.datanow, color='r', label='Animation Data')
        self.axes[1].set_xlim([0, self.x_animation_range[-1]])
        self.axes[1].set_title("Reference Animation Data (" + str(self.x_animation_range.size) + " samples)")
        
        plt_data, = self.axes[2].plot(self.x_data_range, self.datainput, label='Original Data')
        self.axes[2].set_xlim([0, self.x_data_range[-1]])
        self.axes[2].set_ylabel("Amplitude")
        self.axes[2].set_title("Original Data (" + str(self.x_data_range.size) + " samples)")
        
        plt_audio, = self.axes[3].plot(self.x_audio_range, self.dataaudio, label='Audio data', color='black')
        self.axes[3].set_xlim([0, self.x_audio_range[-1]])
        self.axes[3].set_xlabel("Time (s)")
        self.axes[3].set_title("Audio Data (" + str(self.x_audio_range.size) + " samples)")
